...
{
	char *line, *token, *end_tag, *tmp, **tokens, *buf;
	...
	/* Set everything in tokens array to NULL */
	for (i = 0; i < 256; i++)
		tokens[i] = NULL;
	...
	for (i = 0; (token = parse_line(&line)); i++)
	{
		...
		{
			/* Save heredoc symbol to tokens array and go to next index */
			tokens[i++] = token;

			/* next token is heredoc's contents. init to empty string */
			token = _strdup("");
			token_length = 0;
